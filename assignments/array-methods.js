// A local community center is holding a fund raising 5k fun run and 
// has invited 50 small businesses to make a small donation on their behalf
//  for some much needed updates to their facilities. 
//   Each business has assigned a representative to attend the event along 
...
runners.forEach((currentItem) => {
  ...
  fullNames.push(currentItem.first_name, currentItem.last_name);
});
...
// BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. 
// This array will contain just strings.
let firstNamesAllCaps = [];
runners.map((currentItem) => {
  firstNamesAllCaps.push(currentItem.first_name.toUpperCase(), currentItem.last_name);
})
console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due
//  to an ordering issue. We need a filtered version of the 
//  runners array, containing only those runners with large 
//  sized shirts so they can choose a different size. 
...
let runnersLargeSizeShirt = runners.filter((currentItem) =>{
    ...
    // Now that you have used .forEach(), .map(), .filter(), and 
    // .reduce(). 
    //  I want you to think of potential problems you could solve
    //   given the data set and the 5k fun run theme.  
    //   Try to create and then solve 3 unique problems 
